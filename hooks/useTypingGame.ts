...
export const useTypingGame = () => {
    ...
    const startGame = useCallback((newText: string) => {
        clearMainTimer();
    ...
    const wpm = elapsedTime > 0 ? Math.round((typed.length / 5) / (elapsedTime / 60)) : 0;
    const accuracy = totalTyped > 0 ? ((totalTyped - errors) / totalTyped) * 100 : 100;
